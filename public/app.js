...
async function renderMold(id) {
  ...
  app.innerHTML=`<a href="/" class="back">‹ 金型一覧へ</a><section class="mold-identity"><span>入力対象の金型</span><h1>${escapeHtml(mold.name)}</h1><small>管理No. ${id}</small></section><section class="total-card"><span>累計ショット数</span><strong>${formatShots(mold.total_shots)}</strong><small>shots</small></section><div id="message"></div><section class="card accent"><h2>生産終了後のカウンター値を記録</h2><form id="shot-form"><label for="counter-value">金型カウンターの累計値</label><input id="counter-value" class="shot-input" type="number" inputmode="numeric" min="${Number(mold.total_shots)+1}" required autofocus><p class="muted">現在値 ${formatShots(mold.total_shots)} より大きい値を入力</p><button class="primary-wide">累計値を記録</button><div class="compact-options"><details><summary>日付を変更</summary><input id="recorded-on" type="date" value="${today()}" required></details><details><summary>＋ メモを追加</summary><input id="notes" maxlength="500"></details></div></form></section><section class="card maintenance-card"><h2>メンテ記録を追加</h2><form id="maintenance-form"><label for="maintenance-details">実施内容</label><textarea id="maintenance-details" maxlength="500" required placeholder="例：分解清掃、ピン交換、グリスアップ"></textarea><label for="maintenance-on">実施日</label><input id="maintenance-on" type="date" value="${today()}" required><p class="muted">現在の累計 ${formatShots(mold.total_shots)} ショット時点として記録します。</p><button class="primary-wide">メンテを記録</button></form></section><section class="card trend-card"><h2>ショット数とメンテの推移</h2><p class="muted">上に進むほど新しい記録です。オレンジ色がメンテ実施です。</p>${trendHtml(mold.records,mold.maintenances)}</section><section class="card"><div class="actions"><button id="show-qr" class="secondary">QRコードを表示</button><a class="button secondary" href="/scan">別のQRを読む</a></div><div id="qr-area"></div></section><section class="card"><h2>ショット入力履歴</h2>${recordsHtml(mold.records)}</section><section class="card"><h2>メンテ履歴</h2>${maintenanceHtml(mold.maintenances)}</section>`;
  ...
  document.querySelector('#maintenance-form').onsubmit=async e=>{e.preventDefault();const b=e.currentTarget.querySelector('button');b.disabled=true;try{await request(`/api/molds/${id}/maintenances`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({performedOn:document.querySelector('#maintenance-on').value,details:document.querySelector('#maintenance-details').value})});await renderMold(id);setMessage('メンテ記録を追加しました。');}catch(error){b.disabled=false;setMessage(error.message,true);}};
}
...
function maintenanceHtml(records){return records.length?`<ul class="history maintenance-history">${records.map(r=>`<li><div><time>${escapeHtml(formatDate(r.performed_on))}</time><p>${escapeHtml(r.details)}</p></div><strong>${formatShots(r.counter_value)}<small> 時点</small></strong></li>`).join('')}</ul>`:'<p class="muted">まだメンテ記録はありません。</p>';}
function trendHtml(shots,maintenances){
  const chronological=[...shots].reverse();let running=0;
  const shotPoints=chronological.map(r=>{running+=Number(r.shot_count);return{type:'shot',date:formatDate(r.recorded_on),value:Number(r.counter_value)||running,label:`+${formatShots(r.shot_count)}`};});
  const events=[...shotPoints,...maintenances.map(r=>({type:'maintenance',date:formatDate(r.performed_on),value:Number(r.counter_value),label:r.details}))].sort((a,b)=>a.date.localeCompare(b.date)||a.value-b.value);
  if(!events.length)return '<p class="muted">ショットまたはメンテを記録すると、ここに推移が表示されます。</p>';
  const max=Math.max(1,...events.map(e=>e.value));const height=Math.max(440,events.length*82);const top=28,bottom=32,usable=height-top-bottom;
  const positioned=events.map(e=>({...e,y:height-bottom-(e.value/max)*usable,x:e.type==='maintenance'?76:42}));
  const line=shotPoints.map((p,i)=>`${i?'L':'M'}42 ${(height-bottom-(p.value/max)*usable).toFixed(1)}`).join(' ');
  return `<div class="trend-scroll"><svg class="trend-chart" viewBox="0 0 300 ${height}" role="img" aria-label="累計ショット数とメンテナンスの推移"><line x1="42" y1="${top}" x2="42" y2="${height-bottom}" class="trend-axis"/>${line?`<path d="${line}" class="trend-line"/>`:''}${positioned.map(e=>`<g class="trend-event ${e.type}"><circle cx="${e.x}" cy="${e.y}" r="${e.type==='maintenance'?9:6}"/><text x="${e.type==='maintenance'?96:62}" y="${e.y+5}" class="trend-label">${escapeHtml(e.date)}　${e.type==='maintenance'?escapeHtml(e.label.slice(0,18)):`${formatShots(e.value)} shots`}</text></g>`).join('')}</svg></div>`;
}
...
async function start(){if(location.pathname==='/login')return renderLogin();try{const session=await request('/api/auth/session');csrfToken=session.csrfToken;sessionStorage.setItem('csrfToken',csrfToken);}catch(error){if(error.status===401){location.href=`/login?returnTo=${encodeURIComponent(safeReturnPath(location.pathname))}`;return;}throw error;}const match=location.pathname.match(/^\/molds\/(\d+)$/);if(match)return renderMold(match[1]);if(location.pathname==='/scan')return renderScan();return renderHome();}
