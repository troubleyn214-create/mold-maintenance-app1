...
function trendHtml(shots,maintenances){
  ...
  return `<div class="trend-scroll"><svg class="trend-chart" viewBox="0 0 300 ${height}" role="img" aria-label="累計ショット数とメンテナンスの推移"><line x1="42" y1="${top}" x2="42" y2="${height-bottom}" class="trend-axis"/>${line?`<path d="${line}" class="trend-line"/>`:''}${positioned.map(e=>`<g class="trend-event ${e.type}"><circle cx="${e.x}" cy="${e.y}" r="${e.type==='maintenance'?9:6}"/><text x="${e.type==='maintenance'?96:62}" y="${e.y-5}" class="trend-value">${formatShots(e.value)} shots</text><text x="${e.type==='maintenance'?96:62}" y="${e.y+15}" class="trend-label">${escapeHtml(e.date)}${e.type==='maintenance'?`　🔧 ${escapeHtml(e.label.slice(0,18))}`:''}</text></g>`).join('')}</svg></div>`;
}
