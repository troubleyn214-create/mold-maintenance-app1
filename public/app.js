...
async function renderMold(id) {
  ...
  document.querySelector('#shot-form').onsubmit=async(e)=>{e.preventDefault();const b=e.currentTarget.querySelector('button');b.disabled=true;try{await request(`/api/molds/${id}/shots`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({recordedOn:document.querySelector('#recorded-on').value,shotCount:document.querySelector('#shot-count').value,notes:document.querySelector('#notes').value})});await renderMold(id);setMessage('ショット数を記録しました。');}catch(error){b.disabled=false;setMessage(error.message,true);}};
}
function recordsHtml(records){return records.length?`<ul class="history">${records.map(r=>`<li><div><time>${escapeHtml(r.recorded_on)}</time>${r.notes?`<p>${escapeHtml(r.notes)}</p>`:''}</div><strong>+${formatShots(r.shot_count)}</strong></li>`).join('')}</ul>`:'<p class="muted">まだ記録はありません。</p>';}
